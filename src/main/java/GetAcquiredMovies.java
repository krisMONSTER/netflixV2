...
@WebServlet("/GetAcquiredMovies")
public class GetAcquiredMovies extends HttpServlet {
    @Override
        ...
        if(user == null){
            ...
            try {
                ...
                String query = "SELECT v.title FROM video v, services s" +
                        " WHERE s.idAccount=(SELECT id FROM account WHERE login=?) AND v.id=s.idVideo AND s.status=1";
